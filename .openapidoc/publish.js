...
function getConfig() {
  const repo = GitUrlParse(process.env.OA_GIT_URL || "git@github.com:PegaSysEng/eth2signer.git");
  ...
  const specPath = process.env.OA_SPEC_PATH || "../core/build/resources/main/openapi/eth2signer.yaml";
  const specVersion = yaml.safeLoad(fs.readFileSync(specPath, "utf8")).info.version;
  ...
  return {
    spec: {
      ...
      latestDist: path.join(distDir, `${specFileNamePrefix}-latest${specFileNameExtension}`),
      releaseDist: path.join(distDir, `${specFileNamePrefix}-${specVersion}${specFileNameExtension}`)
    },
    distDir,
    versions: {
      url: versionsFileUrl,
...
async function main() {
  ...
  try {
    ...
    log(`OpenAPI spec [${spec.version}] published to [${ghPagesConfig.branch}] using user [${ghPagesConfig.user.name}]`);
  } catch(err) {
    log(`OpenAPI spec [${spec.version}] failed to publish to [${ghPagesConfig.branch}] using user [${ghPagesConfig.user.name}]: ${err.message}`)
    process.exit(1)
  }
}
...
async function fetchVersions(versionsUrl) {
  ...
  throw new Error(`${versionsUrl} fetch failed with status: ${response.statusText}`);
}
